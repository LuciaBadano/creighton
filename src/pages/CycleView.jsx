...
function buildCycles(menstrualStarts) {
  ...
  for (let i = 0; i < menstrualStarts.length; i++) {
    ...
    const end = menstrualStarts[i + 1]
      ? addDays(menstrualStarts[i + 1], -1)
      : null;
    cycles.push({
      number: i + 1,
      startDate: start,
...
export default function CycleView({ onOpenModal }) {
  ...
  const modalYear = modalDate
    ? parseInt(modalDate.split("-")[0])
    : new Date().getFullYear();
  const modalMonth = modalDate
    ? parseInt(modalDate.split("-")[1]) - 1
    : new Date().getMonth();
    ...
    return (
      <div className={styles.empty}>
        ...
        <p className={styles.emptyHint}>
          Registrá el primer día de tu menstruación en la vista mensual para que
          aparezca acá.
        </p>
      </div>
    );
